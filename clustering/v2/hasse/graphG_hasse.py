...
def find_common_descendants(G, nodes):
    if not nodes:
        return set()

    common = nx.descendants(G, nodes[0])
    for node in nodes[1:]:
        common &= nx.descendants(G, node)
...
def build_graph():
    ...
    json_file = os.path.join(script_dir, "hasse_diagrams_n4.json") #connected hasse only
    ...
    if isinstance(hasse_results, dict):
        ...
        raise TypeError("hasse_diagrams_n4.json")
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_file_csv = os.path.join(script_dir, "final_graph_adjacency-hasse_n4.csv")
    ...
    output_file_reduced_csv = os.path.join(script_dir, "final_graph_adjacency-hasse_n4_transitive_reduction.csv")
    ...
    # mat_j = G.nodes[64]['matrix']
    # mat_i = G.nodes[65]['matrix']
    # mat_z = G.nodes[74]['matrix']
    mat_x = G.nodes[134]['matrix']
    mat_y = G.nodes[125]['matrix']
    # print(mat_j)
    # print(mat_i)
    # print(mat_z)
    print(mat_x)
    print(mat_y)
    # common = find_common_descendants(G_reduced, [122, 125, 134])
    # print("Common descendants of 122, 125, 134:", common)
    ...
    output_file_shortest_paths = os.path.join(script_dir, "shortest_path_distances-hasse_n4.csv")
    ...
    output_file_shortest_paths = os.path.join(script_dir, "shortest_path_directed-hasse_n4.csv")
    df_shortest_paths.to_csv(output_file_shortest_paths)
    ...
    output_file_reachability_matrix = os.path.join(script_dir, "reachability_matrix-hasse_n4.csv")
