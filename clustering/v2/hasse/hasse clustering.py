...
mc_json = os.path.join(script_dir, "M_c_matrices_diagonal_1 ('e1', 'e2', 'e5', 'e6') corrupted10%.json")
with open(mc_json, "r") as f:
    M_c_data = json.load(f)
    
    
def plot_Gcp_coverage(G_cp, reach_sets, G, total_patients=80):
...
def find_best_combos(M, T_percent, reach_sets, G, total_patients=80):
    t2 = time.time()
    ...
    timings["build_Gcp_seconds"] = time.time() - t2
...
def display_reachability(selectedData):
    ...
    return f"Selected nodes: {selected_nodes} → Unique M_c reachables: {len(total_reachable_mc)} (~{len(total_reachable_mc)/80:.2%})"
