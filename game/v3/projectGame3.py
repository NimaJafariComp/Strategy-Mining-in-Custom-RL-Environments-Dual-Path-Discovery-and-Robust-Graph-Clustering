...
STATIC_LAYOUT = {
    ...
    "rocks": [
        {"pos": (15, 26), "color": "red", "name": "rock1"},
        {"pos": (17,  5), "color": "red", "name": "rock2"}
    ],
...
def save_screenshot():
    """
    Save two files:
      - screenshotN.png : game view with annotation circles (no legend overlay)
      - legendN.png     : legend panel saved separately
    """
    global offset_x, offset_y

    TYPE_COLORS = {
        "player":      (255, 215, 0),   # gold
        "key":         (30, 144, 255),  # dodger blue
        "explosive":   (255, 69, 0),    # orange-red
        "door":        (0, 206, 209),   # dark turquoise
        "rock":        (128, 128, 128), # gray
        "coin":        (218, 165, 32),  # goldenrod
    }
    CIRCLE_RADIUS = 30
    CIRCLE_THICKNESS = 7

    # Start from a copy of the current screen
    ...
    # Find next index where neither file exists
    index = 1
    while True:
    ...
    # --- Save screenshot (only annotations, no legend panel) ---
    pygame.image.save(base_surface, screenshot_name)

    # --- Render legend to its own image ---
    legend_entries = [
        ("player",    "Player"),
        ("key",       "Key"),
        ("explosive", "Explosive"),
    ...
    for key, label in legend_entries:
        ...
        # text with count (only if >0)
        count = legend_counts[key]
        text = f"{label}" + (f" (x{count})" if count else "")
        txt = font.render(text, True, (0, 0, 0))
        legend_surface.blit(txt, (30, y + row_h // 2 - txt.get_height() // 2))
        y += row_h
...
def init_game_objects():
    ...
    for rock in STATIC_LAYOUT["rocks"]:
        x, y = rock["pos"]
        color = rock["color"]
        name   = rock.get("name", "rock")
        rock_obj = Rock(x * tmx_data.tilewidth * SCALING_FACTOR, y * tmx_data.tileheight * SCALING_FACTOR, load_image("rock.png"), color, name)
        rocks.add(rock_obj)
    ...
    while running:
        ...
        if not inventory_open and keys[pygame.K_e]:
            ...
            for coin in collected_coins:
                ...
                # trigger a win on treasure
                print("🎉 Game Won! Treasure collected.")
                game_won = True
                end_time = time.time()
                step_count = len([e for e in player_data if e.get("event") == "move"])
